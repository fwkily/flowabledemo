...
import com.fwk.flowabledemo.util.CommUtil;
...
public class ExpenseController {
    ...
    /**
     * 获取审批管理列表
     * 直接返回List<Task>会序列化报错，故转为map
     */
    @ApiOperation(value = "获取审批管理列表")
    @GetMapping(value = "/list")
    public List<?> list(String userId) {
        List<Task> tasks = taskService.createTaskQuery().taskAssignee(userId).orderByTaskCreateTime().desc().list();
        String[] s = new String[]{"id","assignee","owner","parentTaskId","name"};
        List<Map<String, Object>> list = new ArrayList<>();
        for (Task task : tasks) {
            System.out.println(task.toString());
            Map<String, Object> map = CommUtil.obj2map(task, s);
            list.add(map);
        }
        return list;
    }
